...
### Redux Toolkit
처음 프로젝트를 시작했을 때 Redux Toolkit을 사용하지 않고 Redux를 사용하였습니다. 
Redux를 사용할 때 보일러 플레이트 코드가 많아져 이리저리 파일을 옮겨다녀야 하는 불편함이 있었습니다. 

파일을 옮겨다니면서 코드를 봐야 하니 코드 예측 가능성이 떨어지고, 또 코드 해석능력이 떨어졌습니다. 
더군다나 Redux-saga를 도입하기로 해 보일러 플레이트 코드는 더 많아졌고, 불편함이 배로 늘어났습니다.

이러한 불편함과 또 보일러 플레이트 코드를 줄이기 위해  Redux-toolkit을 적용시켰고 
Redux-toolkit에서 제공하는 createSlice 하나로 type, action, reducer를 한 파일에서 관리할 수 있었습니다.
